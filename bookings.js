...
export { getAvailability, createBooking, listBookings, getBooking, updateBooking, findConflicts };            ) AS is_booked
     FROM equipment e WHERE e.is_active = 1 ORDER BY e.category, e.name`
  )
    .bind(date)
    .all();
...
async function createBooking(request, env, json) {
  ...
  const result = await env.DB.prepare(
    `INSERT INTO bookings (brand_id, customer_id, event_date, event_type, status, notes)
     VALUES (?, ?, ?, ?, 'inquiry', ?)`
  )
    .bind(brandId, customerId, body.event_date, body.event_type ?? null, body.notes ?? null)
    .run();
