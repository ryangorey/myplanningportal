...
async function createBooking(request, env, json) {
  ...
  return json({ id: result.meta.last_row_id, status: "inquiry", customer_id: customerId }, 201);
}
...
// POST /api/bookings/:id/contract -- sales+ only. Body is the raw file
// bytes (same not-multipart pattern as the brand logo upload); content-type
// header identifies the format. Reuses the same LOGOS R2 bucket branding
// logos live in (just under a "contract-" key prefix) so this doesn't need
// its own bucket -- the existing GET /api/uploads/:key already serves
// anything in that bucket, contract PDFs included.
const MAX_CONTRACT_BYTES = 15 * 1024 * 1024; // 15MB
...
async function uploadBookingContract(request, env, id, json) {
  if (!env.LOGOS) {
    return json({ error: "File storage isn't set up yet (missing the LOGOS R2 binding)." }, 500);
  }

  const booking = await env.DB.prepare("SELECT id FROM bookings WHERE id = ?").bind(id).first();
  if (!booking) return json({ error: "Booking not found." }, 404);

  const contentType = (request.headers.get("content-type") || "").split(";")[0].trim().toLowerCase();
  const ext = ALLOWED_CONTRACT_TYPES[contentType];
  if (!ext) {
    return json({ error: "That doesn't look like a supported file type. Use PDF, PNG, or JPEG." }, 400);
  }

  const bytes = await request.arrayBuffer();
  if (bytes.byteLength === 0) return json({ error: "The uploaded file is empty." }, 400);
  if (bytes.byteLength > MAX_CONTRACT_BYTES) {
    return json({ error: "That file is too large -- keep it under 15MB." }, 400);
  }

  const key = `contract-${id}-${Date.now()}.${ext}`;
  await env.LOGOS.put(key, bytes, { httpMetadata: { contentType } });

  const contractUrl = `/api/uploads/${key}`;
  await env.DB.prepare("UPDATE bookings SET contract_url = ? WHERE id = ?").bind(contractUrl, id).run();

  return json({ id: Number(id), contract_url: contractUrl });
}

export { getAvailability, createBooking, listBookings, getBooking, updateBooking, findConflicts, uploadBookingContract };
