...
import { listMyTasks, listTasksForCustomer, createTask, updateTask, deleteTask } from "./tasks.js";
import { listContactsForCustomer, createContact, deleteContact } from "./client-contacts.js";
import {
  listPipelineStages,
  createPipelineStage,
  updatePipelineStage,
...
export default {
  async fetch(request, env) {
    ...
    // Clients: sales+ only (employees don't get customer PII). Tasks and
    // CRM contact-log entries for a specific client live under here too --
    // both sub-resource checks have to come before the plain id-only GET/
    // PATCH below, or those would swallow the sub-resource requests.
    if (resource === "customers") {
      ...
      if (request.method === "PATCH" && id && !subresource) return addCors(await updateCustomer(request, env, id, json));
      return addCors(json({ error: "Not found." }, 404));
    }

    // A single CRM contact-log entry -- flat top-level resource (like
    // /api/tasks/:id) so deleting one doesn't need a 3-segment path.
    if (resource === "client-contacts" && id) {
      const auth = await requireStaffAdmin(request, env, "sales");
      if (auth.error) return addCors(auth.error);
      if (request.method === "DELETE") return addCors(await deleteContact(env, id, json));
      return addCors(json({ error: "Not found." }, 404));
    }

    // Pipeline stages: the configurable columns of the leads board.
    // Everyone who can work leads (sales+) needs to read the list to render
    // the board, but only admin+ can add/rename/reorder/delete a stage --
    // that's a structural change to how every lead gets tracked, not
    // routine day-to-day lead work.
    if (resource === "pipeline-stages") {
      const readAuth = await requireStaffAdmin(request, env, "sales");
      if (readAuth.error) return addCors(readAuth.error);
      if (request.method === "GET" && !id) return addCors(await listPipelineStages(env, json));

      const auth = await requireStaffAdmin(request, env, "admin");
      if (auth.error) return addCors(auth.error);
      if (request.method === "POST" && !id) return addCors(await createPipelineStage(request, env, json));
      if (request.method === "PATCH" && id && !subresource) return addCors(await updatePipelineStage(request, env, id, json));
      if (request.method === "POST" && id && subresource === "move") return addCors(await movePipelineStage(request, env, id, json));
      if (request.method === "DELETE" && id && !subresource) return addCors(await deletePipelineStage(env, id, json));
      return addCors(json({ error: "Not found." }, 404));
    }

    // Leads: the pipeline itself. Sales+ only -- employees get no
    // visibility here, same bar as Clients/Venues.
    if (resource === "leads") {
      const auth = await requireStaffAdmin(request, env, "sales");
      if (auth.error) return addCors(auth.error);
      if (request.method === "GET" && !id) return addCors(await listLeads(env, url, json));
      if (request.method === "GET" && id) return addCors(await getLead(env, id, json));
      if (request.method === "POST" && !id) return addCors(await createLead(request, env, json));
      if (request.method === "PATCH" && id) return addCors(await updateLead(request, env, id, json));
      if (request.method === "DELETE" && id) return addCors(await deleteLead(env, id, json));
      return addCors(json({ error: "Not found." }, 404));
    }
